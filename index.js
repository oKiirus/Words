...
$(function(){
    $('.input_field').keyup(function(){
        ...
        $('.rep_input').eq(num).html($(this).val())
    })
...
function displayStory() {
    ...
    for(var i = 0; i < story.inputs; i++){
        var inputHtml = `<input type = 'text' class = 'input_field' id = 'input_${i}' placeholder = 'input ${i}'/>`
        $('#input_fields').append(inputHtml)
    }
    $('#bank_words').empty()
    for(var i = 0; i < story.words.length; i++){
        var inputHtml = `<button class = 'word_bank_button'>${story.words[i]}</button>`
        $('#bank_words').append(inputHtml)
    }
}
